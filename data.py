...
# 定义全局变量
PATCH_SIZE = 17  # 切片patch大小
OUTPUT_CLASSES = 16  # 输出9类地物
TEST_FRAC = 0.50  # 用来测试数据的百分比
NEW_DATA_PATH = os.path.join(os.getcwd(), "patch")  # 存放数据路径
...
def loadData(flieName, dataName, labelName):
    ...
    data = np.transpose(data, (2, 0, 1))  # 将通道数提前
    return data, label
...
data, label = loadData("Indian","Indian_pines_corrected.mat","Indian_pines_gt.mat")
